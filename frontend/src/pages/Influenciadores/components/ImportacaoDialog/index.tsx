...
import { influenciadorService, ImportacaoResultadoDTO, ImportacaoLinhaDTO } from '../../service';
import './styles.scss';
...
function ImportacaoDialog({ visible, onHide, onComplete }: ImportacaoDialogProps) {
  ...
  const importar = async () => {
    ...
    setErro(null);
    try {
      const res = await influenciadorService.importarCSV(arquivo);
      setResultado(res);
      setEtapa('resultado');
    } catch {
  ...
  const header = (
    <span className="p-dialog-title">
      <i className="pi pi-file-import" /> Importar Influenciadores
    </span>
  );
      ...
      {etapa === 'upload' && (
        <div className="importacao-upload">
          <div className="upload-instructions">
            ...
            <p>Selecione um arquivo CSV com os dados dos influenciadores</p>
            <button type="button" className="btn-template" onClick={() => influenciadorService.downloadTemplate()}>
              <i className="pi pi-download" /> Baixar modelo de exemplo
            </button>
          </div>

          <div className="upload-area">
            <input ref={inputRef} type="file" accept=".csv,.txt" onChange={onFileSelect} className="file-input" id="csv-upload" />
