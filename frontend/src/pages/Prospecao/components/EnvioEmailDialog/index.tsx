...
import TemplateDialog from '../../../TemplatesEmail/components/TemplateDialog';
...
function EnvioEmailDialog({ visible, onHide, onSaved, onToast, prospecao, tipo, registrarComoFollowUp }: Readonly<Props>) {
  ...
  const podeAddTemplate = canAdd(authUser?.roles ?? [], MODULES.TEMPLATES_EMAIL.prefix);
  ...
  return (
    ...
    >
      ...
      <div className="form-field">
        ...
        <SelectComCadastro onAdd={() => setTemplateDialogVisible(true)} visivel={podeAddTemplate} title="Cadastrar novo template">
    ...
    <TemplateDialog
      ...
      tipoInicial={tipo}
      onSaved={(saved) => {
        queryClient.invalidateQueries({ queryKey: ['templates-por-tipo', tipo] });
